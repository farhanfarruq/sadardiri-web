    ...
    <div class="row">
        ...
        <div class="col-lg-9 col-xl-10 content ms-auto py-4 px-md-4">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h2 class="mb-0 text-gradient">Edit Kebiasaan</h2>
                        ...
                        <div class="row mb-4">
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('icon') is-invalid @enderror" id="icon" name="icon" required>
                                    ...
                                    <option value="fas fa-running" {{ old('icon', $habit->icon) == 'fas fa-running' ? 'selected' : '' }}>🏃‍♂️ Lari</option>
                                    <option value="fas fa-book" {{ old('icon', $habit->icon) == 'fas fa-book' ? 'selected' : '' }}>📚 Baca</option>
                                    <option value="fas fa-utensils" {{ old('icon', $habit->icon) == 'fas fa-utensils' ? 'selected' : '' }}>🍽️ Makan Sehat</option>
                                    <option value="fas fa-dumbbell" {{ old('icon', $habit->icon) == 'fas fa-dumbbell' ? 'selected' : '' }}>🏋️ Olahraga</option>
                                    <option value="fas fa-bed" {{ old('icon', $habit->icon) == 'fas fa-bed' ? 'selected' : '' }}>😴 Tidur Cukup</option>
                                    <option value="fas fa-tint" {{ old('icon', $habit->icon) == 'fas fa-tint' ? 'selected' : '' }}>💧 Minum Air</option>
                                </select>
                                @error('icon')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                @enderror
